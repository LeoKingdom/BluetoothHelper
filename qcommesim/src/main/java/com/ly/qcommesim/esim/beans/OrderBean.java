...
public class OrderBean {
    ...
    public ESimHelper.EsimEnum esimAction() {
        if (data.length > 7) {
            boolean moduleId = data[5] == (byte) 0x30;
            byte eventId = data[6];
            if (moduleId) {
                if (eventId == (byte) 0x08) {
                    return ESimHelper.EsimEnum.ESIM_ACTIVE;
                } else if (eventId == (byte) 0x09) {
                    return ESimHelper.EsimEnum.ESIM_CANCEL;
                } else if (eventId == (byte) 0x0A) {
                    return ESimHelper.EsimEnum.ESIM_DELETE;
                }
